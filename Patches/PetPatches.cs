...
namespace SLCosmetics.Patches
{
    ...
    internal static class UpdateTarget
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            newInstructions.InsertRange(0, new List<CodeInstruction>()
            {
                ...
                new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(PetCosmetic), nameof(PetCosmetic.IsCosmetic), new[] { typeof(ReferenceHub) })),
                new CodeInstruction(OpCodes.Brtrue_S, skip),
                new CodeInstruction(OpCodes.Ldarg_0),
                new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(PetCosmetic), nameof(PetCosmetic.IsCosmetic), new[] { typeof(Scp096TargetsTracker) })),
                new CodeInstruction(OpCodes.Brtrue_S, skip),
            });
    ...
    internal static class UpdateObservers
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            newInstructions.InsertRange(index, new List<CodeInstruction>()
            {
                ...
                new CodeInstruction(OpCodes.Brtrue_S, skip),
                new CodeInstruction(OpCodes.Ldarg_0),
                new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(PetCosmetic), nameof(PetCosmetic.IsCosmetic), new[] { typeof(Scp173ObserversTracker) })),
                new CodeInstruction(OpCodes.Brtrue_S, skip),
            });
    ...
    internal static class OnFootstepPlayed
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            newInstructions.InsertRange(0, new List<CodeInstruction>()
            {
                ...
                new CodeInstruction(OpCodes.Callvirt, AccessTools.Method(typeof(PetCosmetic), nameof(PetCosmetic.IsCosmetic), new[] { typeof(ReferenceHub) })),
    ...
    internal static class RegisterEncounter
    {
        [HarmonyPrefix]
        private static bool Prefix(ReferenceHub target)
        {
            var pet = Exiled.API.Features.Player.Get(target);
            if (pet == null) return false;
            if (pet.IsNPC) return false;
            return true;
        }
